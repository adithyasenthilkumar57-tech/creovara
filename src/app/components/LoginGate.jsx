...
export default function LoginGate({ onLoginSuccess }) {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            const containerEl = rootRef.current.querySelector('.login-container-inner');
            const checkboxEl = rootRef.current.querySelector('.form-container .form-row input[type="checkbox"]');
            const nameEl = rootRef.current.querySelector('.form-container .form-row input[name="name"]');
            const emailEl = rootRef.current.querySelector('.form-container .form-row input[name="email"]');
            const submitBtn = rootRef.current.querySelector('.form-container .form-row input[type="submit"]');

            const sprayer = rootRef.current.querySelector('.sprayer');
            const sprayHandContainer = rootRef.current.querySelector('.spray-hand-container');
            const sprayLines = Array.from(rootRef.current.querySelectorAll('.spray-line'));
            const sprayBubbles = Array.from(rootRef.current.querySelectorAll('.spray-bubble'));

            const pushingHand = rootRef.current.querySelector('.pushing-hand');
            const sprayerHead = rootRef.current.querySelector('.sprayer-head');
            const gearsContainer = rootRef.current.querySelector('.login-svg-root .gears');
            const gearConnector = rootRef.current.querySelector('.gear-connector');

            const pullSystemContainer = rootRef.current.querySelector('.pull-system');

            const checkboxPullLine = rootRef.current.querySelector('.checkbox-pull-line');
            const checkboxPullCircle = rootRef.current.querySelector('.checkbox-pull-circle');
            const btnPullLine = rootRef.current.querySelector('.submit-btn-connector');
            const btnHandlerCircle = rootRef.current.querySelector('.submit-btn-circle');

            const spiralContainer = rootRef.current.querySelector('.spiral-container');
            const weightBigContainer = rootRef.current.querySelector('.weight-big-container');

            const scalesContainer = rootRef.current.querySelector('.scales-container');
            const scalesLine = rootRef.current.querySelector('.scales-moving-line');
            const weightBig = rootRef.current.querySelector('.weight-big');
            const spiralPath = rootRef.current.querySelector('.spiral-path');

            const carContainer = rootRef.current.querySelector('.car-container');
            const car = rootRef.current.querySelector('.car');
            const carInclineWrapper = rootRef.current.querySelector('.car-container g');
            const timingChains = Array.from(rootRef.current.querySelectorAll('.timing-chain'));
            const reelsConnector = rootRef.current.querySelector('.reels-connector');
            const carWeightConnector = rootRef.current.querySelector('.car-weight-connector');

            const grabbingHand = rootRef.current.querySelectorAll('.grabbing-hand');
            const grabbingHandOpenFingers = Array.from(rootRef.current.querySelectorAll('.grabbing-hand-finger-open'));
            const grabbingHandClosedFingers = Array.from(rootRef.current.querySelectorAll('.grabbing-hand-finger-closed'));
            ...
            layoutPreparation();
            scaleToFit();
            // window.onresize = scaleToFit; // Cleaned up via observer or just event listener
            const resizeListener = () => scaleToFit();
            ...
            function createEmailTl() {
                ...
                tl
                    .fromTo(state, { handClosed: false }, { duration: .01, handClosed: true }, ">")
            ...
            const onSubmitClick = () => {
                if (emailValid && checkboxEl.checked && nameValid && sprayRepeatCounter > 1) {
                    ...
                    gsap.to(".form-row", {
                        ...
                        stagger: .1,
                        onComplete: () => {
                            // Call the callback prop to switch view
            ...
            // Cleanup function for listeners (ctx.revert handles gsap)
            return () => {
        ...
        return () => ctx.revert();
    }, [onLoginSuccess]); // re-run if onLoginSuccess changes, though it shouldn't often

    return (
        <div className="login-wrapper" ref={rootRef}>
            <div className="login-container-inner">
                ...
                <svg className="login-svg-root" viewBox="0 0 1000 1000" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <g className="grabbing-hand">
                        ...
                        <path className="grabbing-hand-finger-open"
                    ...
                    <g className="spray-hand-container">
                        ...
                        <g className="sprayer">
                            <g className="sprayer-head">
                                ...
                                <line className="spray-line" stroke="#777777" strokeDasharray="8 5" x1="22.4" y1="14.76" x2="74.27"
                                    y2="25.2" />
                                <line className="spray-line" stroke="#777777" strokeDasharray="8 5" x1="21.51" y1="21.12"
                                    x2="74.27" y2="25.2" />
                                <line className="spray-line" stroke="#777777" strokeDasharray="8 5" x1="21.44" y1="28.26"
                                    x2="74.27" y2="25.2" />
                                <line className="spray-line" stroke="#777777" strokeDasharray="8 5" x1="22.37" y1="35.54"
                                    x2="74.27" y2="25.2" />
                                <line className="spray-line" stroke="#777777" strokeDasharray="8 5" x1="24.21" y1="42.36"
                                    x2="74.27" y2="25.2" />
                                <line className="spray-line" stroke="#777777" strokeDasharray="8 5" x1="24.31" y1="7.78" x2="74.27"
                                    y2="25.2" />
                    ...
                    <g className="grabbing-hand">
                        <g fill="#ffffff">
                            <rect className="grabbing-hand-finger-closed" x="44.79" y="13.38" width="8.42" height="22.15" rx="3.67"
                                transform="translate(20.57 71.26) rotate(-85.25)" />
                            <rect className="grabbing-hand-finger-closed" x="44.08" y="39.17" width="8.42" height="21.47" rx="3.67"
                                transform="translate(-5.44 93.9) rotate(-85.25)" />
                            <rect className="grabbing-hand-finger-closed" x="45.68" y="30.71" width="8.42" height="22.57" rx="3.67"
                                transform="matrix(0.08, -1, 1, 0.08, 3.91, 88.24)" />
                            <rect className="grabbing-hand-finger-closed" x="44.98" y="22.21" width="8.42" height="22.57" rx="3.67"
                                transform="matrix(0.08, -1, 1, 0.08, 11.74, 79.74)" />
                            <path className="grabbing-hand-finger-closed"
                    ...
                    <g className="scales-container">
                        ...
                        <line className="scales-moving-line" x1="147.6" y1="30.52" x2="40" y2="12" strokeWidth="2"
                            markerStart="url(#ball)" markerEnd="url(#ball)" />
